...
*/
//class를 쓰는 이유? 매개변수를 적게 쓰기위해
// Info 클래스에 매개변수가 들어있다, Info클래스를 이용하여 매개변수를 줄일수 있다
//원래 ArrayList였던게 Object로 가면 다시 ArrayList로 다운 캐스팅이 가능하지만
//원래 Object타입은 ArrayList로 Casting이 안된다
public class D07_PhoneBook {
	// 아직 인스턴스가 생기지 않은 클래스를 설계도만 만들어 놓은것
	...
	public D07_PhoneBook() {
        ...
        // 1.
	public boolean addGroup(String groupName) {// 그룹이름이 같으면 어떤일이?
	...
	// 2.
	public void addInfo(String groupName, Info info) {// info는 Info클래스에 인스턴스
	...
	public void searchByPhoneNumber(String numberFrag) {        
		...
		for (String groupName : phonebook.keySet()) {                        
			...
			for (Entry<String, Info> entry : group.entrySet()) { //.replace("-", "") : -를 없애라
